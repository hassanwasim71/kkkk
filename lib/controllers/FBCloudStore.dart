...
class FBCloudStore{
  ...
  static Future<void> dislikeToPost(String postID,MyProfileData userProfile,bool isDisLikePost) async{
    if (isDisLikePost) {
      DocumentReference dislikeReference = Firestore.instance.collection('thread').document(postID).collection('dislike').document(userProfile.myName);
      await Firestore.instance.runTransaction((Transaction myTransaction) async {
        await myTransaction.delete(dislikeReference);
      });
    }else {
      await Firestore.instance.collection('thread').document(postID).collection('dislike').document(userProfile.myName).setData({
        'userName':userProfile.myName,
        'userThumbnail':userProfile.myThumbnail,
      });
    }
  ...
  static Future<void> updatePostDisLikeCount(DocumentSnapshot postData,bool isDisLikePost,MyProfileData myProfileData) async{
    postData.reference.updateData({'postDisLikeCount': FieldValue.increment(isDisLikePost ? -1 : 1)});
    if(!isDisLikePost){
